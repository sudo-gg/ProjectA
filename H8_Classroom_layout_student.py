...
def Classroom(room, username, period, SEND):
    def convToGraphAndFindClosestSeat(root, canv, numOfSeats, seatingArrangement, room, acceptableEdgeLen=70):
        ...
        if classGraph.isConnected():
            # Seat 1 is closest to the door
            try:
                previousNodesDict, shortestPathToVertexDict = classGraph.dijkstrasAlgorithm(1)
                # finds which unbooked seat is closest to seat 1
                # in the form [ (vertex, shortestDist) , (ect, ect) ]
                sortedListOfShortestPathToVertices = insertionSort(shortestPathToVertexDict)
                booked = 0
                for seat in sortedListOfShortestPathToVertices:
                    if str(seat[0]) in seatingArrangement:
                        pass
                    else:
                        # If seat is not booked then they can book it
                        closestSeatNum = seat[0]
                        if messagebox.askokcancel("Confirmation",
                                                  f"Are you sure you want to book Seat {closestSeatNum}?"):
                            seatingArrangement[closestSeatNum] = username
                            canvas.itemconfig(f"Seat{closestSeatNum}", fill="green")
                            writeToCsv(username, closestSeatNum, period, room)
                            seatingArrangement[str(closestSeatNum)] = username
                            booked = True
                            break
                        else:
                            booked = False
                ...
                # Room Full check
                if type(booked) == int:
                    messagebox.showinfo("Room full", "Sorry this room is now full")
            # Other error
            except ValueError:
                print("Error with input to Dijkstra's - likely not valid vertex")
        else:
            acceptableEdgeLen += 50
            classGraph.destroyGraph()
            convToGraphAndFindClosestSeat(root, canv, numOfSeats, seatingArrangement, room, acceptableEdgeLen)
    ...
    # Basic setup for classroom window GUI
    ...
    # Condition here for SEND student - Only available to SEND students due to framing bias (explained in development)
    if SEND == 1:
        shortestPathButton = tk.Button(classroomRoot, text="Book seat with shortest path to door",
                                       command=lambda c=classroomRoot, ca=canvas, n=numOfSeats,
                                                      s=seatingArrangement, r=room: convToGraphAndFindClosestSeat(c, ca,
                                                                                                                  n, s,
                                                                                                                  r))
